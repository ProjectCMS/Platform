        ...
        <li class="list-inline-item dropdown notification-list">
            ...
            <div class="dropdown-menu dropdown-menu-right profile-dropdown ">
                ...
                @if(config('adminlte.logout_method') == 'GET' || !config('adminlte.logout_method') && version_compare(\Illuminate\Foundation\Application::VERSION, '5.3.0', '<'))
                    <a class="dropdown-item" href="{{ url(config('dashboard.logout_url', 'auth/logout')) }}"><i class="dripicons-exit text-muted"></i>
                        Logout
                    </a>
                @else
                    <a class="dropdown-item" href="#" onclick="event.preventDefault(); document.getElementById('logout-form').submit();"><i class="dripicons-exit text-muted"></i>
                        Logout
                    </a>
                    <form id="logout-form" action="{{ url(config('dashboard.logout_url', 'auth/logout')) }}" method="POST" style="display: none;">
                        @if(config('dashboard.logout_method'))
                            {{ method_field(config('dashboard.logout_method')) }}
                        @endif
                        {{ csrf_field() }}
                    </form>
                @endif
